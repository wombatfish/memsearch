...
- **`score`'s scale depends on whether the reranker ran.** With the reranker disabled (library default, `RerankerConfig.model == ""`), `score` is the RRF-fusion score above, normalized to `[0, 1]` via `store.py`'s `max_rrf = len(reqs) / (rrf_k + 1)`. With the reranker enabled (the Claude Code plugin's default), `reranker.py`'s `_rerank_onnx`/`_rerank_torch` overwrite `score` with a cross-encoder relevance score instead — sigmoid for single-logit models, softmax for 2-logit models — a differently-shaped `[0, 1]` number with no relationship to RRF rank. Guidance that gates on `score` with a fixed numeric cutoff would silently mean different things depending on reranker config; prefer relative comparisons over fixed cutoffs.
...
- **Search daemon restart requirement.** `memsearch watch` hosts the warm search daemon in-process. After deploying a new CLI build, restart any live watcher; an already-running watcher keeps executing the old installed code and will not host new daemon behavior until it is restarted.
- **Daemon-served searches use watcher config.** `memsearch search` / `expand` try the watch-hosted daemon first for local ONNX/local providers. The daemon uses the watcher's resolved config, not one-off CLI overrides from the client process. Pass `--no-daemon` when an invocation must honor local overrides such as reranker or graph settings.
- **Daemon auth is local-only.** The daemon uses `multiprocessing.connection` authkeys, whose challenge uses MD5 HMAC internally. Treat it as a same-user local gate, not as a multi-user or compliance-grade security boundary.

**Plugin hooks.** Refresh with `/plugin marketplace update <mp>` → `/plugin install memsearch@<mp>` → `/reload-plugins`.
- ⚠️ **Same version-keying trap on the plugin side.** If `plugins/claude-code/.claude-plugin/plugin.json` version is unchanged, `/plugin install` reports `already installed globally` and may not refresh the cache. **Bump the plugin.json version** to force a new cache dir, then verify by grepping the *cached* hook (not the repo copy) for your change. **When the change must NOT bump the version** (i.e. it isn't a genuine upstream port pass — see Versioning & Release above), `claude plugin update <plugin>` is version-keyed too and will report "already at the latest version," silently skipping the cache refresh even though the underlying files changed, for a directory-source (local dev) marketplace. Use `claude plugin uninstall <plugin>` followed by `claude plugin install <plugin>` instead — it re-copies from source regardless of whether the version moved. Either way, verify by grepping the new text directly in the *cached* file path (e.g. `~/.claude/plugins/cache/<marketplace>/memsearch/<version>/...`), not the repo copy, and don't just trust the CLI's success message.
