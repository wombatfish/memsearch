...
# macOS lacks `timeout` â€” use perl alarm(2) as a portable fallback with a 2-second deadline.
...
MEMORY_DIR="$MEMSEARCH_DIR/memory"`n`n# Repo-bucket subdirectory under MEMORY_DIR.`n# In global mode many repos share MEMORY_DIR; segregate by main-repo name so`n# search results can be filtered/grouped and parallel sessions on different`n# repos never write the same daily file. Worktrees collapse to their main repo`n# via --git-common-dir (returns the SHARED .git path; its parent is the main`n# worktree root). Sessions outside any git repo bucket to __no_repo__.`n_REPO_BUCKET="__no_repo__"`n_GIT_COMMON_DIR="$(git rev-parse --git-common-dir 2/dev/null || echo "")"`nif [ -n "#!/usr/bin/env bash
# Shared setup for memsearch command hooks.
# Sourced by all hook scripts â€” not executed directly.
...
start_watch() {
  ...
  # Server (http/tcp): setsid â€” watch runs persistently for real-time indexing.
...
GIT_COMMON_DIR" ]; then`n  # Resolve to absolute path (--git-common-dir may be relative like ".git")`n  if [ -d "#!/usr/bin/env bash
# Shared setup for memsearch command hooks.
# Sourced by all hook scripts â€” not executed directly.
...
GIT_COMMON_DIR" ]; then`n    _GIT_COMMON_DIR="$(cd "#!/usr/bin/env bash
# Shared setup for memsearch command hooks.
# Sourced by all hook scripts â€” not executed directly.
...
GIT_COMMON_DIR" && pwd)"`n    _MAIN_ROOT="$(dirname "#!/usr/bin/env bash
# Shared setup for memsearch command hooks.
# Sourced by all hook scripts â€” not executed directly.
...
GIT_COMMON_DIR")"`n    _REPO_BUCKET="$(basename "#!/usr/bin/env bash
# Shared setup for memsearch command hooks.
# Sourced by all hook scripts â€” not executed directly.
...
MAIN_ROOT" | tr '[:upper:]' '[:lower:]')"`n  fi`nfi`nMEMORY_BUCKET_DIR="$MEMORY_DIR/#!/usr/bin/env bash
# Shared setup for memsearch command hooks.
# Sourced by all hook scripts â€” not executed directly.
