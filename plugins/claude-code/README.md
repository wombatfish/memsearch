...
4. **Injects cold-start context.** Returns recent memory as `additionalContext`. When the maintenance task has written curated memory files (`CORRECTIONS.md`/`PROJECT.md`/`USER.md`), it injects those (priority order, byte-capped) plus a reserved, recency-biased recent window — the newest and previous dated daily logs, each tail-truncated to its newest turns, newest nearest the prompt. Without curated files it falls back to the 2 most recent daily logs. This gives Claude awareness of recent sessions, which helps it decide when to invoke the memory-recall skill.
