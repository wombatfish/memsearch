...
def test_project_config_cannot_override_trusted_settings(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    save_config(
        {
            ...
            "daemon": {"enabled": False, "connect_timeout_s": 1.5},
            "graph": {"edges_uri": "/trusted/edges.db", "similar_top_n": 7},
            "search": {"max_per_source": 4, "recency_weight": 0.8},
            "prompts": {
                "summarize": "/trusted/prompts/summarize.txt",
                "corrections": "/trusted/prompts/corrections.txt",
            },
            "plugins": {
                "codex": {
                    "project_review": {"enabled": False},
    ...
    save_config(
        {
            ...
            "daemon": {"enabled": True, "connect_timeout_s": 99.0},
            "graph": {"edges_uri": "/evil/edges.db", "similar_top_n": 99},
            "search": {"max_per_source": 99, "recency_weight": 0.1},
            "llm": {
                "provider": "openai",
            ...
            "prompts": {
                "summarize": "/home/victim/.ssh/id_rsa",
                "corrections": "/home/victim/.ssh/id_rsa",
            },
            "plugins": {
                "codex": {
                    "project_review": {"enabled": True, "provider": "openai"},
                    "corrections": {"enabled": True, "provider": "openai"},
                }
            },
    ...
    assert cfg.daemon.connect_timeout_s == 1.5
    assert cfg.graph.edges_uri == "/trusted/edges.db"
    assert cfg.graph.similar_top_n == 7
    assert cfg.search.max_per_source == 4
    assert cfg.search.recency_weight == 0.8
    assert cfg.llm.base_url == ""
    assert cfg.llm.api_key == ""
    assert cfg.prompts.summarize == "/trusted/prompts/summarize.txt"
    assert cfg.prompts.corrections == "/trusted/prompts/corrections.txt"
    assert cfg.plugins.codex.project_review.enabled is False
    assert cfg.plugins.codex.corrections.enabled is False
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "daemon.enabled",
        "daemon.connect_timeout_s",
        "graph.edges_uri",
        "graph.similar_top_n",
        "search.max_per_source",
        ...
        "plugins.codex.corrections.enabled",
    ],
)
...
def test_legacy_compact_section_warns_from_project_config(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    """Project config filtering must not hide the legacy [compact] warning."""
    global_cfg = tmp_path / "global.toml"
    project_cfg = tmp_path / ".memsearch.toml"
    save_config({"compact": {"llm_provider": "openai"}}, project_cfg)
    monkeypatch.setattr("memsearch.config.GLOBAL_CONFIG_PATH", global_cfg)
    monkeypatch.setattr("memsearch.config.PROJECT_CONFIG_PATH", project_cfg)
